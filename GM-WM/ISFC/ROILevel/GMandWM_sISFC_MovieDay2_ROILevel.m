...
for roii = 1:image_dim(2)
    ...
    for roij = 1:image_dim(2)
        sISFC_subpair_MovieDay2_z(:,:,roii,roij) = atanh(corr(squeeze(MovieDay2_GM_ROILevel(:,roii,:))',squeeze(MovieDay2_WM_ROILevel(:,roij,:))'));
   
        tmp = ones(image_dim(1),image_dim(1));
        tmp = diag(diag(tmp));
        
        ma = sISFC_subpair_MovieDay2_z(:,:,roii,roij);
        ma(tmp==1) = 0;
        
        sumval = sum(sum(ma));
        num = 171*(1+171)/2 * 2;
        sISFC_MovieDay2_z(roii,roij) = sumval/num;
    end
    toc
end
...
Input = zeros(size(sISFC_MovieDay2_r,1)+ex,size(sISFC_MovieDay2_r,2)+ex);
Input = Input + NaN;
Input(1:200,1+ex:200+ex) = sISFC_MovieDay2_r;
...
saveas(gcf,'/home/brain/HuPeng_Project/Analysis/GMandWM/ISFC/ROILevel/sISFC_MovieDay2_r.png');
saveas(gcf,'/home/brain/HuPeng_Project/Analysis/GMandWM/ISFC/ROILevel/sISFC_MovieDay2_r.fig');
close all;

save('/home/brain/HuPeng_Project/Analysis/GMandWM/ISFC/ROILevel/sISFC_subpair_MovieDay2_r','sISFC_subpair_MovieDay2_r');
save('/home/brain/HuPeng_Project/Analysis/GMandWM/ISFC/ROILevel/sISFC_MovieDay2_r','sISFC_MovieDay2_r');
...
% for roii = 1:image_dim(2)
%    for roij = 1:image_dim(2)
%       ma = squeeze(sISFC_subpair_MovieDay2_z(:,:,roii,roij));
%       meanvalue = sISFC_MovieDay2_z(roii,roij);
%       
%       big = 0;
%       small = 0;
%       for count = 1:5000
...
% end
% 
% save('/home/brain/HuPeng_Project/Analysis/GMandWM/ISFC/ROILevel/p_s','p');
