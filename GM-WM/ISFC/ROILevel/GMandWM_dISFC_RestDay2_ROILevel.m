...
for roii = 1:image_dim(2)
    ...
    for roij = 1:image_dim(2)
        ...
        for step = 1:image_dim(3)-30+1
        	dISFC_series(:,:,step) = corr(squeeze(RestDay2_GM_ROILevel(:,roii,step:step+30-1))',squeeze(RestDay2_WM_ROILevel(:,roij,step:step+30-1))');
        end
        
        dISFC_subpair_RestDay2(:,:,roii,roij) = std(dISFC_series,0,3);
         
        tmp = ones(image_dim(1),image_dim(1));
        tmp = diag(diag(tmp));
        
        ma = dISFC_subpair_RestDay2(:,:,roii,roij);
        ma(tmp==1) = 0;
        
        sumval = sum(sum(ma));
        num = 171*(1+171)/2 * 2;
        dISFC_RestDay2(roii,roij) = sumval/num;
    end
    toc
end
...
Input = zeros(size(dISFC_RestDay2,1)+ex,size(dISFC_RestDay2,2)+ex);
Input = Input + NaN;
Input(1:200,1+ex:200+ex) = dISFC_RestDay2;
...
saveas(gcf,'/home/brain/HuPeng_Project/Analysis/GMandWM/ISFC/ROILevel/dISFC_RestDay2.png');
saveas(gcf,'/home/brain/HuPeng_Project/Analysis/GMandWM/ISFC/ROILevel/dISFC_RestDay2.fig');
close all;

save('/home/brain/HuPeng_Project/Analysis/GMandWM/ISFC/ROILevel/dISFC_subpair_RestDay2','dISFC_subpair_RestDay2');
save('/home/brain/HuPeng_Project/Analysis/GMandWM/ISFC/ROILevel/dISFC_RestDay2','dISFC_RestDay2');
