...
for neti = 1:image_dim_GM(2)
    ...
    for netj = 1:image_dim_WM(2)
        sISFC_subpair_MovieDay1_z(:,:,neti,netj) = atanh(corr(squeeze(MovieDay1_GM_NetworkLevel(:,neti,:))',squeeze(MovieDay1_WM_NetworkLevel(:,netj,:))'));

        tmp = ones(image_dim_GM(1),image_dim_GM(1));
        tmp = diag(diag(tmp));
        
        ma = sISFC_subpair_MovieDay1_z(:,:,neti,netj);
        ma(tmp==1) = 0;
        
        sumval = sum(sum(ma));
        num = 171*(1+171)/2 * 2;
        sISFC_MovieDay1_z(neti,netj) = sumval/num;
    end
    toc
end
...
saveas(gcf,'/home/brain/HuPeng_Project/Analysis/GMandWM/ISFC/NetworkLevel/sISFC_MovieDay1_r.png');
saveas(gcf,'/home/brain/HuPeng_Project/Analysis/GMandWM/ISFC/NetworkLevel/sISFC_MovieDay1_r.fig');
close all;

save('/home/brain/HuPeng_Project/Analysis/GMandWM/ISFC/NetworkLevel/sISFC_subpair_MovieDay1_r','sISFC_subpair_MovieDay1_r');
save('/home/brain/HuPeng_Project/Analysis/GMandWM/ISFC/NetworkLevel/sISFC_MovieDay1_r','sISFC_MovieDay1_r');

% % Bootstrapping
% for neti = 1:image_dim_GM(2)
%    for netj = 1:image_dim_WM(2)
%       ma = squeeze(sISFC_subpair_MovieDay1_z(:,:,neti,netj));
%       meanvalue = sISFC_MovieDay1_z(neti,netj);
%       
%       big = 0;
%       small = 0;
%       for count = 1:5000
