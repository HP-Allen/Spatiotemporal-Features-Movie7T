...
% Get masked data
% RestDay2_WM_ROILevel : subject x ROI x Time
for subi = 1:length(subfolder)
    ...
    for roii = 1:length(ROIn)
        RestDay2_WM_ROILevel(subi,roii,:) = mean(Data(find(mask==ROIn(roii)),:),1);
    end
    toc
end

save('/home/brain/HuPeng_Project/Analysis/WM/Timeseries/ROILevel/RestDay2_WM_ROILevel','RestDay2_WM_ROILevel');
