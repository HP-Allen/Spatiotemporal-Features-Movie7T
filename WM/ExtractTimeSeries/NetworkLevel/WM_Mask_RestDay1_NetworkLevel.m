...
% Get masked data
% RestDay1_WM_NetworkLevel : subject x Network x Time
for subi = 1:length(subfolder)
    ...
    for Networki = 1:length(Networkn)
        RestDay1_WM_NetworkLevel(subi,Networki,:) = mean(Data(find(mask==Networkn(Networki)),:),1);
    end
    toc
end

save('/home/brain/HuPeng_Project/Analysis/WM/Timeseries/NetworkLevel/RestDay1_WM_NetworkLevel','RestDay1_WM_NetworkLevel');
