...
% Get masked data
% MovieDay2_GM_NetworkLevel : subject x Network x Time
for subi = 1:length(subfolder)
    ...
    for Networki = 1:length(Networkn)
        MovieDay2_GM_NetworkLevel(subi,Networki,:) = mean(Data(find(mask==Networkn(Networki)),:),1);
    end
    toc
end

save('/home/brain/HuPeng_Project/Analysis/GM/Timeseries/NetworkLevel/MovieDay2_GM_NetworkLevel','MovieDay2_GM_NetworkLevel');
